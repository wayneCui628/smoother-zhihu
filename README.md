...
- 🌊 **整批单帧注册（防加载风暴）**：MutationObserver 观测到新一批回答后，在同一帧内批量完成全部注册，立即恢复真实文档高度；杜绝"新行被折叠在种子高度 → 文档变短 → 知乎无限滚动哨兵持续命中 → 反复触发 loading"的深页正反馈风暴；
- 📐 **注册零强制布局**：新卡片注册路径只写 class 不读几何，矩形信息全部由 IntersectionObserver 回调免费提供；批量巡检严格读写分离，150+ 回答深页单次巡检不再触发逐行同步回流；
- 🩺 **SPA 视图自愈看门狗**：深链接回答页只有单条回答、或"查看全部回答"整树换视图时，心跳检测自动重新挂载追踪（健康后降频为近零开销的谓词检查），无需刷新页面即可恢复冻结虚拟化；
...
| 🧠 **离屏节点渲染冻结率** | `0%` (全量参与绘制) | **`96%` (深度冻结)** | 🎯 **GPU 负荷暴跌** | 解除脱离视口卡片的 GPU 显存纹理与合成图层（Layers）绑定 |
| 🎞️ **长流滚动平均帧率 (Scroll FPS)** | 波动顿挫 | **稳定满帧** | ⚡ **丝滑顺畅** | 浏览器合成管线零冗余重绘，彻底告别掉帧与卡死 |
| 🌊 **连续加载风暴收敛 (10 轮×5 回答)** | `511.9 ms`/轮 | **`77.9 ms`/轮** | 🚀 **提速 6.6×** | 整批单帧注册，深页连续追加无折叠残留、无 loading 风暴 |
